...
exports.AGENT_TIMEOUT_MS = exports.MAX_EVALUATOR_AGENTS = void 0;
exports.deduplicateAndPrioritize = deduplicateAndPrioritize;
exports.buildDiffHunksForSurface = buildDiffHunksForSurface;
exports.buildAgentContexts = buildAgentContexts;
exports.buildS5SpawnPlan = buildS5SpawnPlan;
exports.initSpawnedAgent = initSpawnedAgent;
exports.markAgentCompleted = markAgentCompleted;
exports.markAgentTimedOut = markAgentTimedOut;
exports.allAgentsExited = allAgentsExited;
exports.findTimedOutAgents = findTimedOutAgents;
exports.findCoverageGaps = findCoverageGaps;
exports.formatS5FailureJson = formatS5FailureJson;
// ---------------------------------------------------------------------------
// Constants
// ---------------------------------------------------------------------------
/**
 ...
 */
exports.MAX_EVALUATOR_AGENTS = 8;
/**
 ...
 */
exports.AGENT_TIMEOUT_MS = 5 * 60 * 1000; // 5 minutes
/**
 ...
 */
function inferSurfaceType(surfaceId) {
 ...
 */
function surfacePriority(surfaceId) {
    const type = inferSurfaceType(surfaceId);
    const idx = SURFACE_TYPE_PRIORITY.findIndex((t) => type === t || type.startsWith(t));
 ...
 */
function deduplicateAndPrioritize(impactedEntries, maxAgents = exports.MAX_EVALUATOR_AGENTS) {
    ...
    const sorted = [...uniqueSurfaces].sort((a, b) => surfacePriority(a) - surfacePriority(b));
 ...
 */
function buildDiffHunksForSurface(surfaceId, impactedEntries) {
    const hunks = [];
    for (const entry of impactedEntries) {
        if (!entry.surface_ids.includes(surfaceId))
            continue;
        hunks.push({
            file: entry.file,
 ...
 */
function buildAgentContexts(params) {
    const { impactedEntries, targetDomain, sessionId, maxAgents } = params;
    const deduplication = deduplicateAndPrioritize(impactedEntries, maxAgents ?? exports.MAX_EVALUATOR_AGENTS);
    const contexts = deduplication.surface_ids.map((surfaceId) => ({
 ...
 */
function buildS5SpawnPlan(impactedEntries, targetDomain, sessionId, maxAgents = exports.MAX_EVALUATOR_AGENTS) {
    ...
    console.log(`S5: ${deduplication.total_unique} unique surface${deduplication.total_unique === 1 ? "" : "s"} from impacted_entries`);
    if (deduplication.capped_count > 0) {
        console.warn(`S5: capping at ${maxAgents} — dropped ${deduplication.capped_count} surface${deduplication.capped_count === 1 ? "" : "s"}: ` +
            deduplication.dropped_surfaces.join(", "));
    }
    else {
        console.log(`S5: spawning ${contexts.length} evaluator agent${contexts.length === 1 ? "" : "s"} (cap: ${maxAgents})`);
    }
    return {
        agents: contexts,
 ...
 */
function initSpawnedAgent(context) {
    return {
 ...
 */
function markAgentTimedOut(agent) {
    return {
        ...agent,
        status: "timed_out",
        exited_at: Date.now(),
 ...
 */
function allAgentsExited(agents) {
    return agents.every((a) => a.status === "completed" || a.status === "timed_out" || a.status === "skipped");
}
/**
 ...
 */
function findTimedOutAgents(agents, timeoutMs = exports.AGENT_TIMEOUT_MS, nowMs = Date.now()) {
    return agents.filter((a) => a.status === "running" && nowMs - a.spawned_at >= timeoutMs);
}
/**
 ...
 */
function findCoverageGaps(agents, coveredSurfaces) {
    return agents
 ...
 */
function formatS5FailureJson(error) {
...
//# sourceMappingURL=evaluator-dispatch.js.map