...
exports.submitPRReview = submitPRReview;
// ---------------------------------------------------------------------------
// Constants
// ---------------------------------------------------------------------------
const MAX_RETRIES = 3;
const BASE_DELAY_MS = 1_000; // 1 second base for exponential backoff
const EMPTY_BODY = "Bob diff review complete. No findings in impacted surfaces.";
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
/**
 ...
 */
async function createReviewWithBackoff(octokit, owner, repo, pull_number, body, comments) {
    let lastError;
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
        try {
            ...
            if (status === 429) {
                ...
                console.warn(`[reviews-api] GitHub rate limit (429) on attempt ${attempt + 1}/${MAX_RETRIES + 1}. ` +
    ...
    throw new Error(`[reviews-api] createReview failed after ${MAX_RETRIES + 1} attempts (429 rate limit). ` +
...
async function submitPRReview(octokit, owner, repo, pull_number, comments, summary) {
    ...
    try {
        const reviewUrl = await createReviewWithBackoff(octokit, owner, repo, pull_number, reviewBody, ghComments);
        console.log(`[reviews-api] Review posted with ${comments.length} inline comment(s): ${reviewUrl}`);
        return reviewUrl;
    }
    catch (err) {
        const status = getStatusCode(err);
        ...
        // ------------------------------------------------------------------
        console.warn("[reviews-api] 422 Unprocessable on initial review batch. " +
        ...
        console.warn("[reviews-api] All comment positions invalid — falling back to body-only review.");
        const inlinedBody = buildBodyFallback(reviewBody, comments);
        const reviewUrl = await createReviewWithBackoff(octokit, owner, repo, pull_number, inlinedBody, []);
        console.log(`[reviews-api] Recovery review posted with ${comments.length} finding(s) as body text ` +
 ...
 */
async function partitionCommentsByValidity(octokit, owner, repo, pull_number, comments) {
    const valid = [];
    const invalid = [];
    for (const comment of comments) {
        const isValid = await probeComment(octokit, owner, repo, pull_number, comment);
        if (isValid) {
            valid.push(comment);
        }
        else {
 ...
 */
async function probeComment(octokit, owner, repo, pull_number, comment) {
    try {
        await createReviewWithBackoff(octokit, owner, repo, pull_number, `[Bob probe] Validating diff position for ${comment.path}:${comment.position}`, [comment]);
        return true;
    }
    catch (err) {
        const status = getStatusCode(err);
        ...
        console.warn(`[reviews-api] Unexpected error probing ${comment.path}:${comment.position} ` +
 ...
 */
function buildBodyFallback(reviewBody, comments) {
    const sections = comments.map((c, i) => `### Finding ${i + 1}: \`${c.path}\` (position ${c.position})\n\n${c.body}`);
    return [reviewBody, "", ...sections].join("\n\n");
}
//# sourceMappingURL=reviews-api.js.map