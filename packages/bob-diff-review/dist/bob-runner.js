...
exports.buildClaudeChildEnv = buildClaudeChildEnv;
...
const findings_serializer_js_1 = require("./findings-serializer.js");
// ---------------------------------------------------------------------------
// Constants
// ---------------------------------------------------------------------------
/** Total time allowed for the headless claude process to exit. */
exports.BOB_RUNNER_TIMEOUT_MS = 10 * 60 * 1000; // 10 minutes
 ...
 */
class BobRunnerError extends Error {
 ...
 */
function buildClaudeChildEnv(opts) {
    ...
    Object.assign(childEnv, buildClaudeAuthEnv({
 ...
 */
function buildMcpConfig(serverPath) {
    return {
        mcpServers: {
 ...
 */
function validateDiffReviewFindings(value) {
    ...
    if (typeof obj["target_domain"] !== "string" || obj["target_domain"].length === 0) {
        throw new TypeError(`diff-review-findings.json: required string field "target_domain" is missing or empty`);
    }
    ...
    for (const key of ["session_id", "generated_at"]) {
        if (obj[key] !== undefined && typeof obj[key] !== "string") {
            throw new TypeError(`diff-review-findings.json: "${key}" must be a string when present`);
        }
    ...
    if (obj["impacted_entries"] === undefined) {
        ...
        throw new TypeError(`diff-review-findings.json: "impacted_entries" must be an array`);
    }
    // findings must be an array (may be empty).
    if (!Array.isArray(obj["findings"])) {
        throw new TypeError(`diff-review-findings.json: "findings" must be an array`);
    }
 ...
 */
function validateFindingEntry(entry, index) {
    ...
    if (typeof e["file"] !== "string" || e["file"].length === 0) {
        throw new TypeError(`${prefix}: field "file" must be a non-empty string, got ${typeof e["file"]}`);
    }
    // Essential numeric field — the inline position needs a start line.
    if (typeof e["line_start"] !== "number" || !Number.isFinite(e["line_start"])) {
        throw new TypeError(`${prefix}: field "line_start" must be a finite number, got ${typeof e["line_start"]}`);
    }
    // Severity enum — drives the comment body and the Check Run conclusion.
    if (!findings_serializer_js_1.VALID_SEVERITIES.includes(e["severity"])) {
        throw new TypeError(`${prefix}: field "severity" must be one of ${findings_serializer_js_1.VALID_SEVERITIES.join(", ")}, ` +
    ...
    for (const key of ["surface_id", "title", "description", "evidence", "hunk_text"]) {
        if (e[key] === undefined || e[key] === null) {
            e[key] = "";
        }
        else if (typeof e[key] !== "string") {
            throw new TypeError(`${prefix}: field "${key}" must be a string when present, got ${typeof e[key]}`);
        }
    }
    ...
    else if (typeof e["line_end"] !== "number" || !Number.isFinite(e["line_end"])) {
        throw new TypeError(`${prefix}: field "line_end" must be a finite number when present, got ${typeof e["line_end"]}`);
    }
}
// ---------------------------------------------------------------------------
...
function quoteSkillPromptValue(value) {
...
async function runBobDiffReview(params) {
    ...
    if (mcpServerPath && fs.existsSync(mcpServerPath)) {
        ...
        fs.writeFileSync(mcpConfigPath, JSON.stringify(buildMcpConfig(path.resolve(mcpServerPath))), "utf8");
        ...
        process.stderr.write(`[bob-runner] Bob MCP wired (PATH A enabled): ${mcpServerPath}\n`);
    }
    else {
        process.stderr.write(`[bob-runner] BOB_MCP_SERVER_PATH not set or file missing — skill will use PATH B (heuristic dispatch)\n`);
    }
    ...
    if (timedOut) {
        throw new BobRunnerError({
            message: `Bob diff-review process timed out after ${timeoutMs / 1000}s ` +
                `(limit: ${exports.BOB_RUNNER_TIMEOUT_MS / 1000}s). ` +
    ...
    if (exitCode !== 0) {
        const stderrSnippet = stderrOutput.length > 2000
            ? stderrOutput.slice(-2000)
            : stderrOutput;
        throw new BobRunnerError({
            message: `Bob diff-review process exited with code ${exitCode ?? "(killed)"}` +
    ...
    try {
        ...
        const isNotFound = readErr instanceof Error &&
            readErr.code === "ENOENT";
        throw new BobRunnerError({
            message: isNotFound
    ...
    if (parsed !== null && typeof parsed === "object" && !Array.isArray(parsed)) {
        const p = parsed;
        if (typeof p["target_domain"] !== "string" || !p["target_domain"]) {
    ...
    try {
        ...
        throw new BobRunnerError({
            message: `diff-review-findings.json schema validation failed: ${errMsg}`,
            exitCode: 0,
            signal: null,
            stderr: stderrOutput,
            timedOut: false,
...
//# sourceMappingURL=bob-runner.js.map