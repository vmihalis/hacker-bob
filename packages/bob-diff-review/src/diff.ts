...
// Re-export types so callers can import everything from "diff.js".
export type { DiffPositionMap, DiffHunk, PositionEntry } from "./types.js";

// ---------------------------------------------------------------------------
// Minimal Octokit interface
// ---------------------------------------------------------------------------
...
export async function fetchPRDiff(
  ...
  if (typeof diff !== "string") {
    throw new Error(
      `fetchPRDiff: expected string response from GitHub API, got ${typeof diff}`
    );
  }

  if (diff.length === 0) {
    console.warn(
      `[bob-diff-review] fetchPRDiff: PR #${pullNumber} returned an empty diff`
    );
  }
  ...
  if (diff.length > LARGE_DIFF_THRESHOLD) {
    console.warn(
      `[bob-diff-review] fetchPRDiff: diff is ${diff.length} bytes ` +
 ...
 */
export function buildDiffPositionMap(unifiedDiff: string): DiffPositionMap {
  const result: DiffPositionMap = new Map();

  // Normalize CRLF → LF so the regex always works.
  const lines = unifiedDiff.replace(/\r\n/g, "\n").split("\n");

  let currentFile: string | null = null;
  let fileMap: Map<number, number> | null = null;
  let isBinary = false;
