 ...
 */
export function formatCommentBody(finding: FindingEntry): string {
 ...
 */
export function formatPRLevelBody(finding: FindingEntry, filePath: string): string {
 ...
 */
function sideFromKey(key: number): "RIGHT" | "LEFT" {
...
export function resolvePosition(
  ...
  lineStart: number
): { position: number; side: "RIGHT" | "LEFT" } | null {
  ...
  let forwardBest: { key: number; position: number } | null = null;
  let backwardBest: { key: number; position: number } | null = null;

  for (const [key, pos] of fileMap) {
    if (key <= 0) continue; // Skip deletion keys for the forward/backward search.
    if (key >= lineStart) {
      // Forward candidate.
...
export function resolveFindings(
  ...
  for (const finding of findings.findings) {
    ...
    const normalizedFinding: FindingEntry = { ...finding, line_start: lineStart };
