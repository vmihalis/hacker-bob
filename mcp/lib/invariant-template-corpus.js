...
const TEMPLATES = Object.freeze([
  ...
  Object.freeze({
    ...
    foundry_test_template: [
      ...
      "    // CROSS-STACK TARGET BINDING: emit the executed target's address and a sha256 of its",
      "    // runtime bytecode BEFORE the gated call. The VIOLATED (positive) arm fails AT the",
      "    // expectRevert'd call, so a post-call emission would never fire on it; emitting first",
      "    // guarantees the binding is logged regardless of outcome. The runner cross-checks this",
      "    // sha256 against eth_getCode at the same address+fork_block (a fake-deployed or vm.etch'd",
      "    // target has different runtime bytecode and is refused). This line is part of the pinned",
      "    // generated source (generated_source_hash), so an agent-authored harness cannot strip it.",
      "    console.log(string.concat(\"BOB_TARGET_BIND:\", vm.toString(address({TARGET_CONTRACT})), \":\", vm.toString(sha256(address({TARGET_CONTRACT}).code))));",
