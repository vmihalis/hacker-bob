...
module.exports = Object.freeze({
  ...
  description:
    ...
    "AIM-EXEMPTION + INERT: poll fetches the constant Bob-owned sink directly (no targetDomain so the first-party scope check is skipped) but with blockInternalHosts:true + followRedirects:false + NO session egress proxy. When the sink is not configured in the MCP launch env (BOB_OOB_HOST / BOB_OOB_POLL_URL), it returns blocked_by_infra and signs nothing (ships dormant). LOCUS SAFETY: accepts ONLY target_domain + token_handle — no surface_id, URL, host, token value, or severity; the surface + in-scope target are read from the mint binding (the tightest anti-retarget shape).",
  inputSchema: {
    type: "object",
    properties: {
  ...
  session_artifacts_written: ["offensive-runs.jsonl", "offensive-runs/", "oob-interactions.jsonl"],
});
