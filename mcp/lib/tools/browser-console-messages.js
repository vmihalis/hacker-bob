...
async function handler(args = {}) {
  ...
  try {
    ...
    const result = await callBrowser("console_messages", sessionId, {
...
module.exports = Object.freeze({
  name: "bob_browser_console_messages",
  description:
    "Return browser-captured console messages since session start (or since the optional since_index). Each entry: { type, text, location, timestamp }.",
  inputSchema: {
    type: "object",
    properties: {
      target_domain: { type: "string" },
      session_id: { type: "string" },
      since_index: { type: "number" },
    },
