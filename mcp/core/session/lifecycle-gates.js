...
function gateVerifyToGrade(context) {
  const blockers = [];
  try {
    require("../verification/verification.js").requireVerificationCompleteForGrade(context.target_domain);
  } catch (error) {
  ...
  try {
    const { capabilityBlockerCeilingViolations } = require("../frontier/reachability-ceiling.js");
...
function sandboxIsolationBlockersForReportableVerdictClaims(targetDomain) {
  let decision;
  try {
    decision = require("../verdict-sandbox-gate.js").evaluateVerdictSandboxGate(targetDomain);
  } catch (error) {
    return [{
      ...
      message:
        ...
        + compactError(error),
      error: compactError(error),
      remediation: require("../verdict-sandbox-gate.js").SANDBOX_REMEDIATION,
    }];
  }
  ...
  if (decision.decision === "downgrade") {
    ...
    try {
      require("../verdict-sandbox-gate.js").emitSandboxDowngradeWarning(decision, "VERIFY -> GRADE:");
    } catch {
...
function gateClaimFreezeToVerify(context) {
  ...
  let claims;
  try {
    claims = require("../claims/claims.js").readCandidateClaims(context.target_domain);
  } catch (error) {
  ...
  const { assertExploitedClaimHasProof } = require("../claims/claims.js");
...
function gateSetupToOpenFrontier(context) {
  ...
  let evaluation;
  try {
    evaluation = require("../waves/scheduler-preconditions.js").evaluateSchedulerPrecondition(
...
function gateOpenFrontierToClaimFreeze(context) {
  ...
  let chainEvaluation;
  try {
    chainEvaluation = require("../waves/scheduler-preconditions.js").evaluateSchedulerPrecondition(
      "chain_work_terminal",
      { target_domain: context.target_domain },
    );
  } catch (error) {
  ...
  let cellClosure;
  try {
    cellClosure = require("../waves/scheduler-preconditions.js").evaluateSchedulerPrecondition(
  ...
  let seedProducers;
  try {
    seedProducers = require("../waves/scheduler-preconditions.js").evaluateSchedulerPrecondition(
  ...
  let unscannedBodies;
  try {
    unscannedBodies = require("../waves/scheduler-preconditions.js").evaluateSchedulerPrecondition(
...
function gateVerificationReopen(context) {
  ...
  if (!attemptId) {
    try {
      const verification = require("../verification/verification.js");
