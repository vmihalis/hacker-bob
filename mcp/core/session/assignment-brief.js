...
} = require("../frontier/lead-intake.js");
const {
...
} = require("../../domains/blockchain/smart-contracts/evm-rpc-pool.js");
const {
...
} = require("../waves/wave-brief-derivation.js");
const {
...
} = require("../../domains/repo/oss-rootcause-family-corpus.js");
const {
  CLI_TOOL_PACKS,
  fillInvocationPlaceholders,
  observationList,
  selectCliToolPacks,
} = require("../dispatch/cli-tool-packs.js");
const {
...
const {
  ...
  presenceCachePath,
} = require("../dispatch/cli-tool-presence.js");
const {
  repoEnvPath,
} = require("../io/paths.js");
const {
...
function deriveCellFloorForSurface({ domain, surfaceObj, surfaceId, coverageSummary, remainingDepth, maxChildren, belief }) {
  ...
  const { deriveChildFanoutPlan, fanoutPlanningKey } = require("../capability/capability-pack-derivation.js");
  const {
...
function enumerateTransitionCellFloor({ domain, coverageRecords, maxChildren }) {
  const { readTransitionProposals } = require("../waves/task-graph-events.js");
  const { TRANSITION_BUG_CLASS_AXIS } = require("../capability/capability-packs.js");
  const { planTransitionCellsForEdge, fanoutPlanningKey } = require("../capability/capability-pack-derivation.js");
...
function cellFloorPlanningKeysForSurface(domain, surfaceId) {
  const { readCellProposals } = require("../waves/task-graph-events.js");
  const { fanoutPlanningKey } = require("../capability/capability-pack-derivation.js");
  const keys = new Set();
...
function buildChildFanoutPlanForSurface({ domain, surfaceObj, surfaceId, coverageSummary, wave = null, remainingDepthOverride = null }) {
  let policy;
  try {
    policy = require("../io/queue-policy.js").loadQueuePolicy(domain);
  } catch {
    return null;
  }
  ...
  const hostId = require("../io/runtime-resources.js").runtimeClient();
  ...
  const grantedByChild = new Set(require("../dispatch/role-model.js").mcpToolNamesForRole(SPAWN_SUBAGENT_TYPE));
