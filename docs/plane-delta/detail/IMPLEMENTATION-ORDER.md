...
**PR8 — S13 (untrusted-content envelope).** Pred S10 (shipped) — **parallel to PR5–7**. New `mcp/core/untrusted-envelope.js` (nonce per `session-cap.js`; note the `envelope.js` name collision is the unrelated ToolError wrapper).
...
3. **PR2 step (b): re-port the I9 producer** from blob `d1647c0`(+`1a456f1`) into `mcp/core/frontier/reachability.js`; the consumers in `assignment-brief.js:134-141`, `wave-brief-derivation.js`, `dashboard.js:186-189` are already wired and inert — restoring the producer makes them live.
