...
Use `bounty_read_state_summary` for routine decisions. Use `bounty_read_session_state` only when full arrays are needed.

## Resume
- `resume [domain]` accepts one optional non-flag token: `force-merge`.
- If `state.pending_wave` is null, continue from `state.phase`.
- If `state.pending_wave` is non-null, call `bounty_apply_wave_merge({ target_domain, wave_number: state.pending_wave, force_merge })`.
...
3. Tier 2 browser signup: call `bounty_auto_signup({ target_domain, signup_url, email, password, role: "attacker" })`; if auth is stored, continue to verification, otherwise use returned errors/fallback to decide Tier 3.
4. Tier 3 assisted manual: ask the user to register with the temp email/password, then poll/extract verification mail and store auth with `bounty_auth_store`.
5. Tier 4 manual token capture: if the user skips or automation fails, ask the user to log in, open DevTools Console, paste this snippet, then send the copied JSON. Store it with `bounty_auth_store`.
```javascript
(() => {
  const d = {
...
After any successful signup, poll email up to 12 times, extract a code/link, complete verification through `bounty_http_scan`, then repeat the flow for a `victim` role with a new temp email. Verify auth with `bounty_http_scan` against a protected endpoint and call `bounty_transition_phase({ target_domain, to_phase: "HUNT", auth_status })`.

## PHASE 3: HUNT
Read `attack_surface.json` and `bounty_read_state_summary` before every wave. Treat MCP ranking fields as additive. `explored` means completed surface IDs only; `dead_ends` and `waf_blocked_endpoints` are endpoint/path exclusions only; `lead_surface_ids` route later waves.
...
4. Spawn hunters only after `bounty_start_wave` succeeds.
...
Wave: w[wave]
Agent: a[agent]
First action: call bounty_read_hunter_brief({ target_domain: '[domain]', wave: 'w[wave]', agent: 'a[agent]' }).
Use surface_type, bug_class_hints, high_value_flows, evidence, coverage_summary, traffic_summary, audit_summary, circuit_breaker_summary, ranking_summary, intel_hints, and static_scan_hints as prioritization inputs for this one assigned surface.
...
Wave reconciliation:
1. Call `bounty_apply_wave_merge({ target_domain, wave_number: state.pending_wave, force_merge })`.
...
Wave decisions use `bounty_wave_status({ target_domain })`:
...
## PHASE 8: EXPLORE
On user request after REPORT, call `bounty_transition_phase({ target_domain, to_phase: "EXPLORE" })`, read `attack_surface.json` and `bounty_read_state_summary`, run the same wave system and launch barrier as HUNT, then transition to CHAIN and run CHAIN → VERIFY → GRADE → REPORT on all findings.
