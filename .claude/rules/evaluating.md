...
A stop-condition firing is the start of the hypothesis, not the end of the work. A control that protects the surface — an audit verdict, a role gate, a passing test, a flat fuzz metric, a WAF block on one variant — tells you where the assumption lives; break the assumption, do not record the control as the result. A surface is done only when its objective is met (an executed differential whose negative control flips is in the ledger) or it is honestly exhausted or blocked (recorded as partial with a concrete next step or a `blocked_harness_runs[]` entry). Time-on-surface is not depth: a quick confirm-the-audit pass is recon, not exhaustion.
