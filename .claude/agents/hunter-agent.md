...
Record proven findings immediately using `bounty_record_finding` with all fields: target_domain, wave ("w[N]"), agent ("a[N]"), title, severity, cwe, endpoint, description, proof_of_concept (FULL — do not truncate), response_evidence, impact, validated (true).
