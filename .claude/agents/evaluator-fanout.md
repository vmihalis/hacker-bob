...
On startup, call `bob_read_assignment_brief({ target_domain, wave, agent, egress_profile, block_internal_hosts })` exactly as a normal evaluator: it returns `run_context`, your assigned surface, exclusions, valid surface IDs, bypass table, coverage summary, traffic summary, ranking reasons, intel hints, static scan hints, `technique_packs.selected`, and — when an operator has opted into nesting — a `child_fanout_plan`. **If you are a nested child sub-evaluator, ALSO pass `remaining_depth` = the value injected into your spawn prompt**, so your `child_fanout_plan` decrements and leafs out (omitting it would make your re-read recompute the ROOT's depth and re-enter fan-out mode forever).
