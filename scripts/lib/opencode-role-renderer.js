...
// drift-free frontmatter. MCP `bob_*` tools are NOT gated here — they surface
// under a server-namespaced key whose per-agent allow-list grammar is not
// reliably documented, so Bob relies on the MCP server's authoritative
// role-bundle permissions instead (see docs/ADAPTERS.md Capability Rule).
...
// by an `@bob-<name>` mention from the orchestrator body. Per-chain evaluator
// specs are spliced in from evaluatorRoleSpecs() so adding a chain pack
// auto-extends this set without editing this file. No `model:` is pinned —
// OpenCode is BYOK and each subagent inherits the operator's configured model.
// Role IDs must match mcp/lib/role-model.js exactly.
...
// Each {{SPAWN_*}} token becomes an `@bob-<role>` mention block. OpenCode
// resolves the mention against the committed subagent file (whose body is the
// full role contract), so the orchestrator only needs the run-specific header
// plus a wait line. This replaces Claude's `Agent(subagent_type: "...", ...)`
// Task-tool form, which OpenCode's task tool cannot target for custom agents.
...
function applyOpencodeHostText(document) {
  return document
    ...
    .replace(/Evaluator waves MUST use the host's asynchronous\/background worker mechanism when available\./g, "Evaluator waves run as sequential `@bob-<evaluator_agent>` sub-sessions; OpenCode custom subagents are not background-dispatchable, so dispatch one assigned surface per sub-session and rely on the MCP wave-merge barrier for settlement.")
    .replace(/Use each assignment's `evaluator_agent` as the subagent type and its `handoff_token` only in its spawn prompt\./g, "Address each evaluator sub-session to `@bob-<assignment.evaluator_agent>` and include only that assignment's `handoff_token` in its run header.")
    .replace(/Generic evaluator spawn template \(uses the routed `assignment\.evaluator_agent`; the brief itself carries chain-specific context\):/g, "Generic evaluator spawn template (addresses `@bob-<assignment.evaluator_agent>`; the brief itself carries chain-specific context):")
    ...
    .replace(/wait for background completion notifications/g, "wait for each evaluator sub-session to return and its handoff to finalize")
...
function renderOpencodeEvaluatorPackCatalogue() {
  ...
  const lines = packs.map((pack) =>
    `- \`capability_pack: "${pack.id}"\` (chain_family \`${pack.spawn.chain_family}\`) -> evaluator_agent \`${pack.evaluator_agent}\` (address @bob-${pack.evaluator_agent}). chain_id: ${pack.spawn.chain_id_description}. Workflow: ${pack.spawn.workflow_summary} CLI dependency: ${pack.spawn.cli_dependency}; blocked_harness_runs[] kind: ${pack.spawn.blocked_harness_kind_options}.`,
  );
  return [
