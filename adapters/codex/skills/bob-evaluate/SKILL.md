...
- Signed offensive producers — when you have proven or strongly suspect a primitive of these classes on the routed surface, confirm it with the matching producer. Each one server-mints its own payload/origin/canary, runs a fail-closed differential (it signs NOTHING unless the bug is actually proven, so calling one on a non-vulnerable surface is safe and cheap), and on success writes a tamper-evident `offensive-runs` row carrying a server-stamped `demonstrated_severity` you must not override — the strongest evidence form, so prefer it over a hand-written PoC for these classes. Pass ONLY the selectors shown (never raw payloads, URLs, emails, or operator data):
  ...
  - Blind out-of-band injection (SSRF / RCE / XXE / blind SQLi callback): mint a sink token with `bob_oob_mint({ target_domain, surface_id, oracle_kind: "out_of_band_interaction" })`, plant the returned token in the candidate sink with `bob_http_scan` (keep the injection in the audited ledger — never curl), then poll with `bob_oob_poll({ target_domain, token_handle })`. Requires the OOB sink to be configured in the MCP env; on `blocked_by_infra`, record a `blocked_prereqs[]` / `blocked_harness_runs[]` entry and prove impact another way.
