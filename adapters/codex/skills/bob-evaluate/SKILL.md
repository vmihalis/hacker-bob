...
- There is no "global section" concept in a bob report — to the parser an H2 is a finding. The sole exemption (`mcp/lib/audit-report-parser.js:166`) is positional AND prefix-anchored: the heading must START WITH `Summary`, `Overview`, `Introduction` or `Scope` AND arrive while `currentFinding == null`.
