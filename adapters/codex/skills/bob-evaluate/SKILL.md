...
- IDOR / broken access control: `bob_http_confirm` is the negative baseline scan (it mints no signed row). The proof is `bob_http_idor_confirm` — a second-test-identity cross-tenant read of an attacker-owned synthetic object (MEDIUM signed row). A direct object id that 403s under your own profile is not a dead end: try the same object id under the victim profile and through `bob_http_idor_confirm`.
