...
- Signed offensive producers are the PRIMARY way to prove a suspected web primitive on the routed surface, and the gap they close is under-firing — when a surface signal suggests a primitive, route it through the matching producer (detailed below) rather than settle for a hand-rolled `bob_http_scan` differential, which never substitutes for the producer's signed, tamper-evident proof; do not wait until you have already proven the bug to call one. Three roles: (1) the two read-only differential producers `bob_http_xss_reflect` and `bob_http_cors_confirm` are fail-closed — they sign nothing unless a live control-vs-probe differential proves the bug, so a non-mint means "not proven here", not "the surface is clean" — and carry no setup prerequisites (no provisioning, no PII handling, no target-side write), though each still emits audited probe traffic and persists a local proof artifact on mint. That combination makes them safe to CALL proactively, but only on a CONCRETE signal (a specific reflected query param that looks like an XSS sink, or a response echoing the request `Origin` into `Access-Control-Allow-Origin`), never as an ambient sweep across every parameter; because each call still costs requests, treat `coverage_summary` / `audit_summary` / `circuit_breaker_summary` as a precondition and don't re-fire on an already-tested param/endpoint or a host tripping WAF blocks or 429s. A `bob_http_xss_reflect` mint is discovery-only (lift it before reporting); a `bob_http_cors_confirm` mint is itself the reportable MEDIUM. (2) The escalation producer `bob_http_xss_confirm` is not a proactive call: fire it only to lift an existing reflect mint to HIGH (it drives Bob's headless browser, a contended slot). (3) The gated / out-of-band producers `bob_http_massread_confirm`, `bob_http_idor_confirm`, and the `bob_oob_mint` / `bob_oob_poll` pair are NOT free to fire — each needs a concrete hypothesis plus its stated prerequisites and does not inherit the proactive pair's eagerness. On success a producer writes a tamper-evident `offensive-runs` row carrying a server-stamped `demonstrated_severity` you must not override — the strongest evidence form for these classes. When you CALL a producer, pass ONLY the selectors shown — never raw attack payloads, target URLs, emails, or operator data as inputs (the producer mints and returns any token/URL you then plant):
