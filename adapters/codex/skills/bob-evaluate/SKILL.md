...
Other flags: `--no-auth` skips authenticated capture in SETUP and routes the session through SETUP -> OPEN_FRONTIER with `auth_status: "unauthenticated"`; `--private-targets` declares operator intent to use the separately environment-attested loopback/RFC1918 lab target and MUST make the SETUP `bob_init_session` call include `lab_authorization: { private_targets: true }` plus `allow_internal_hosts: true` (it grants nothing unless the MCP server also has the exact `BOB_LAB_TARGET_ACK` and matching `BOB_LAB_TARGET`; never infer or add this flag for a public target); `--deep` enables broader script-heavy seed mapping plus durable surface-lead promotion; `--egress <profile>` uses a named operator-managed egress profile, defaulting to `default`; `--block-internal-hosts` forces strict direct-egress DNS/private/internal-host blocking for MCP HTTP tools; `--allow-internal-hosts` disables the paranoid default only for explicitly authorized internal/lab programs.
...
**Seed mapping.** On a fresh run, do not read or glob protected session files before initialization; call `bob_init_session` through MCP first. A `session-read-guard` denial of raw file access is not evidence that MCP is unavailable and is not a reason to ask the operator to re-authorize an already-attested target. Call `bob_init_session({ target_domain, target_url, deep_mode, checkpoint_mode, egress_profile, block_internal_hosts, allow_internal_hosts, lab_authorization })`, omitting `block_internal_hosts` unless `--block-internal-hosts` was supplied, omitting `allow_internal_hosts` unless `--allow-internal-hosts` or `--private-targets` was supplied, and omitting `lab_authorization` unless `--private-targets` was explicitly supplied. For `--private-targets`, pass exactly `allow_internal_hosts: true, lab_authorization: { private_targets: true }`; the server-side env attestation remains the independent authorization gate. Use `result.data.state.target` as the canonical `target_domain` after initialization and `result.data.state.block_internal_hosts` as the effective value for later calls.
...
```text
For each plan angle, use Codex spawn_agent for surface-discovery-agent -> Codex worker.
- agent_type: "worker"
...
<!-- @precondition: unscanned_bodies_drained -->
...
Id-bearing crown surfaces earn done through a live cross-tenant differential, not recon. When your assigned surface is `id_bearing` (an object/account/tenant-scoped endpoint whose access must differ by principal), BEFORE you close anything obtain a second authenticated principal and run the differential — the proactive complement to the finalize gate, which already rejects `surface_status: complete` on an id-bearing surface that has no executed cross-tenant sweep.
- Call `bob_list_auth_profiles`. If ≥2 distinct authenticated principals exist, proactively run `bob_run_auth_differential` across them on the frozen id-bearing endpoints to earn a per-endpoint `cross_tenant_flip`: principal A reads its own object while a distinct principal B is denied that same object (the negative control flips 2xx->4xx across identities). A one-principal replay, a coverage row, or a `bypass_attempts` narrative does NOT earn it.
- If only one principal exists, provision a second before closing: capture a second principal's credential via the browser record->flush flow (`bob_browser_session_start_recording` -> `bob_browser_flush_recorded_requests`) and promote it into a named auth profile with `bob_auth_store` (e.g. `profile_name: "victim"` or `"tenant_b"`), so `bob_run_auth_differential` can flip across the two. The orchestrator SETUP normally provisions the `victim` profile (via `bob_auto_signup`); this in-wave `bob_auth_store` promotion is the recovery path when that step did not run. Reach the crown with the authenticated `bob_browser_session_*` tools and the UA-settable `bob_ws_probe` when the impact lives past HTTP-only tools.
- If a second principal genuinely cannot be obtained, record an honest `blocked_prereqs[]` entry of kind `auth_missing` naming the un-run cross-tenant test and set `surface_status: partial`. Never mark an id-bearing crown `complete` on unauthenticated recon or a coverage/bypass narrative — a missing second principal is partial, not done.
...
- Second-order / stored effect (inject-here, surfaces-there): a value you write at one endpoint that later surfaces at a DISTINCT read endpoint (profile→admin panel, comment→moderation queue, import→preview/render). `bob_secondorder_mint({ target_domain, surface_id, oracle_kind: "second_order_reread", injection_locus, observation_locus })` server-mints a 256-bit canary + a silent decoy bound to distinct in-scope injection/observation endpoints and returns only the benign `canary_payload` (it never signs); inject that payload into the injection endpoint via a target-facing tool, then `bob_secondorder_reread({ target_domain, token_handle, expect: "interaction" })` — Bob RE-READS the bound observation endpoint itself (safeFetch) and signs the exploited_safely row ONLY when the exact canary is present as a parsed LEAF and the server-secret decoy is absent (MEDIUM ceiling, masked oracle). Run the decoy-silent control (mint a SEPARATE binding you do NOT inject, `expect:"silence"`) and pair the two via `bob_verify_finding_differential` — the flip earns the verified_pass. If the surfaced canary EXECUTES, drive `bob_http_xss_confirm` for the HIGH proof instead.
...
Every new reportable web finding must include `request_method` plus a structured `injection_point`. For a GraphQL surface, also include `graphql_operation` plus `graphql_resolver`; do not freeze a GraphQL claim with either field missing.
...
For this surface the bar is a `bob_foundry_run` (concrete fork) or `bob_halmos_run` (symbolic) whose assertion fails on the live fork and holds on the control. `bob_halmos_run` reaches invariant violations a single concrete fork run cannot exhaust (signature-replay variants, oracle-staleness boundaries, donation/rounding edge cases, integer-overflow conditions).

Consider — any one of these is a way in; pick by the source you read, do not walk them in order: admin-EOA compromise, governance-proposal bypass, signature replay/forgery, oracle staleness/manipulation, delegated-role drift, upgrade-path takeover, bridge replay, chain-ID confusion, donation/rounding, precision loss, hook/callback abuse, malicious ERC20, flash-loan-callable entry. Use the conditions in `bob_spec_status.trust_assumptions[*].bypass_conditions` when present; otherwise derive them from the source. For each you pursue, articulate the concrete state machine the bypass exercises before you scaffold a test for it.

Prerequisites (causal — these are the orderings the work imposes, not a checklist; do each before what depends on it):
- Fetch the assigned contract's verified source via `bob_evm_fetch_source`. Read the source files from `[SESSION]/contracts/<chain_id>/<address>/sources/` to map external entry points, role-gated functions, callouts (oracles, bridges, hooks), and storage layout — you cannot scaffold a test against source you have not fetched.
- Build the live trust map. For every privileged role / `wards` mapping you find, call `bob_evm_role_table` to enumerate current members on a recent block. Cross-reference with `bob_spec_status.trusted_roles[].bypass_conditions`.
- Scaffold a Foundry test under `harness_path/test/` (use `Write` for the `.t.sol` file). The test forks the assigned chain at a recent block and exercises the hypothesis. Pin `--fork-block-number` so the run is reproducible by the verifier.
- Run the test via `bob_foundry_run`. Inspect `tests[].status`, `reason`, `gas_used`, and `counterexample`. If `ok: false` with `reason: forge_not_in_path`, `reason: "rpc_unreachable"`, a reason starting with `no_fork_endpoints`, populated `rpc_policy_rejections[]`, or all `fork_attempts[]` failed with RPC errors, set `surface_status: partial` and record `blocked_harness_runs[]` with `kind: foundry_fork` or `rpc_endpoint` as appropriate.

Recording contract (the gate's input format):
- Record a `bypass_attempts[]` entry for every condition you actually attempted, citing the actual harness path + test name in `attempt_summary`. `outcome` follows the run: `no_finding` if the assertion held, `partial_evidence` if you observed an unexpected state but didn't reach a fund-loss condition, `finding_recorded` (with `finding_id`) when you recorded a finding via `bob_record_candidate_claim`, or `blocked` when the harness couldn't run.

Cross-surface pivots and composition conjectures:
- When the assigned contract exposes a credible cross-stack pivot that crosses a trust boundary (a bridge mint/burn edge, an L1↔L2 messenger, an IBC/XCM relayer hop, a cross-chain message verifier, a web→on-chain identity or value path), call `bob_propose_transition` with `from_surface`, `to_surface`, a closed-enum `transition_kind`, and a bounded `trust_assumption`. The orchestrator routes the Transition node into the affected Surface briefs.
- When you form a composition conjecture worth dispatching — two surfaces whose individual findings or behaviors compose into impact neither reaches alone — call `bob_propose_hypothesis` with a `hypothesis_statement` describing the A→B composition and `surface_refs` listing the surfaces touched, then call `bob_attach_contract` to bind the witness predicates + production paths + invariants to the minted node and clear the dispatcher's satisfiability gate.
...
- `bob_read_static_analysis_index({ target_domain })` surfaces pre-computed static hits (Slither / semgrep / solhint findings the inventory walker already ran) keyed to source files — read it first so flagged reentrancy / unchecked-call / access-control lines seed a concrete bypass condition instead of a cold source read. When a local analyzer produced a SARIF report (a `--build` Slither/semgrep run, an imported CI artifact), ingest it via `bob_ingest_sarif({ target_domain, ... })` so those results join the index and become bypass-loop leads.
...
For this surface the bar is a `bob_anchor_run` whose test passes under the evaluator convention (`tests[].status: Pass` = bug reproduced) on the live program/fork and holds on the control.

Consider — any one of these is a way in; pick by the IDL and accounts you read, do not walk them in order: missing_signer check, account_validation gap, owner-check absent, cpi_privilege_escalation via signed seeds reused, upgrade_authority_compromise, arbitrary_invoker via raw `invoke`, realloc_drain via adversary-supplied lamports, close_account_drain on missing ownership check, token_account_substitution, sysvar_tampering, discriminator_collision, reentrancy_via_cpi, rent_exemption_drain, unrestricted_authority. Use the conditions in `bob_spec_status.trust_assumptions[*].bypass_conditions` when present; otherwise derive them from the IDL. For each you pursue, articulate the concrete instruction sequence the bypass exercises before you scaffold a test for it.

Prerequisites (causal — these are the orderings the work imposes, not a checklist; do each before what depends on it):
- Fetch the assigned program's upgrade authority via `bob_svm_fetch_program` and (if present in the brief) IDL via `bob_svm_fetch_account`. Read the IDL fields to map instructions, expected signer accounts, expected owner accounts, PDA seeds, and account constraints — you cannot scaffold a test against a program whose accounts you have not read.
- Build the live trust map. For every privileged role / multisig PDA you find, call `bob_svm_fetch_account` on the multisig data account and decode its members list. Cross-reference with `bob_spec_status.trusted_roles[].bypass_conditions`. Confirm `program.upgrade_authority` either matches a multisig or is null (frozen).
- Scaffold an Anchor test under `harness_path/tests/` (use `Write` for the `.ts` file). The test boots a local validator (or clones from mainnet via `solana-test-validator --clone <program> --url <fork>`) and exercises the hypothesis. Pin a `fork_slot` when slot-dependent state matters; for slot-agnostic invariants leave it null and the verifier re-runs against current state.
- Run the test via `bob_anchor_run`. Inspect `tests[].status` (`Pass` = bug reproduced under the evaluator convention), `reason`, `duration_ms`. If `ok: false` with `reason: anchor_not_in_path`, `reason: "rpc_unreachable"`, a reason starting with `no_fork_endpoints`, populated `rpc_policy_rejections[]`, or all `fork_attempts[]` failed with RPC errors, set `surface_status: partial` and record `blocked_harness_runs[]` with `kind: anchor_fork` or `rpc_endpoint` as appropriate.

Recording contract (the gate's input format):
- Record a `bypass_attempts[]` entry for every condition you actually attempted, citing the actual harness path + test name in `attempt_summary`. `outcome` follows the run: `no_finding` if the assertion held, `partial_evidence` if you observed an unexpected state but didn't reach a fund-loss condition, `finding_recorded` (with `finding_id`) when you recorded a finding via `bob_record_candidate_claim`, or `blocked` when the harness couldn't run.

Cross-surface pivots and composition conjectures:
- When the assigned program exposes a credible cross-stack pivot that crosses a trust boundary (a bridge mint/burn edge, a Wormhole/cross-chain message verifier, an attestation or relayer hop, a CPI into another in-scope program, a web→on-chain identity or value path), call `bob_propose_transition` with `from_surface`, `to_surface`, a closed-enum `transition_kind`, and a bounded `trust_assumption`. The orchestrator routes the Transition node into the affected Surface briefs.
- When you form a composition conjecture worth dispatching — two surfaces whose individual findings or behaviors compose into impact neither reaches alone — call `bob_propose_hypothesis` with a `hypothesis_statement` describing the A→B composition and `surface_refs` listing the surfaces touched, then call `bob_attach_contract` to bind the witness predicates + production paths + invariants to the minted node and clear the dispatcher's satisfiability gate.
...
- `bob_read_static_analysis_index({ target_domain })` surfaces pre-computed static hits (cargo-geiger / clippy / Anchor-lint findings the inventory walker already ran) keyed to source files — read it first so flagged missing-signer / account-validation / owner-check lines seed a concrete bypass condition instead of a cold IDL read. When a local analyzer produced a SARIF report (a `--build` analyzer run, an imported CI artifact), ingest it via `bob_ingest_sarif({ target_domain, ... })` so those results join the index and become bypass-loop leads.
...
For this surface the bar is a `bob_aptos_run` or `bob_sui_run` whose test passes under the evaluator convention (`tests[].status: Pass` = bug reproduced) and holds on the control.

Consider — any one of these is a way in; pick by the ABI / module surface you read, do not walk them in order. Use the conditions in `bob_spec_status.trust_assumptions[*].bypass_conditions` when present; otherwise derive them from the ABI. Move bug class catalog:
...
For each condition you pursue, articulate the concrete entry-function call sequence the bypass exercises before you scaffold a test for it.

Prerequisites (causal — these are the orderings the work imposes, not a checklist; do each before what depends on it):
- Enumerate the assigned package's surface area. Aptos: call `bob_aptos_fetch_module` for each module on the address; read `abi.exposed_functions` (entry functions are the attack surface), `abi.structs[]` (capability types like `Capability`, `BurnCap`, `MintCap`, `KeyedAuthorityCap`), and `abi.friends[]` (intra-package privilege grants). Sui: call `bob_sui_fetch_package` to enumerate `<module>.exposedFunctions[]` and `<module>.structs[]` (key/store abilities). Cross-reference with `bob_spec_status.trust_assumptions[]` — you cannot scaffold a test against a package whose surface you have not enumerated.
- Build the live trust map. For every privileged capability / shared object / treasury you find, fetch its current state via `bob_aptos_fetch_resource` (Aptos) or `bob_sui_fetch_object` (Sui). On Sui specifically, decode the `owner` field — `Immutable` and `Shared` objects have different attack profiles than `AddressOwner` / `ObjectOwner`. Confirm `package upgrade_policy` either matches an UpgradeCap held by a multisig or is `Immutable` / sealed.
- Scaffold a Move test under `harness_path/sources/` (use `Write` for the `.move` file). Use `#[test]` for pure-VM tests, `#[test_only]` for setup helpers. Aptos tests run inside a deterministic VM with no real network access — `aptos move test --filter` does NOT clone mainnet state. Sui tests use `test_scenario::Scenario` to simulate transactions; `sui move test --filter` similarly runs offline. For both, the `match_test` filter you record in `sc_evidence` MUST match the test function name (Aptos: `module_name::test_name`; Sui: `test_function_name` matched against a regex).
- Run the test via `bob_aptos_run` or `bob_sui_run`. Inspect `tests[].status` (`Pass` = bug reproduced under the evaluator convention), `tests[].test_id`, `tests[].reason`. If `ok: false` with `reason: aptos_not_in_path` / `sui_not_in_path` / `aptos_dependency_missing` / `sui_dependency_missing` / `move_compile_failed`, `reason: "rpc_unreachable"`, a reason starting with `no_fork_endpoints`, populated `rpc_policy_rejections[]`, or all `fork_attempts[]` failed with RPC errors, set `surface_status: partial` and record `blocked_harness_runs[]` with `kind: aptos_fork`, `sui_fork`, or `rpc_endpoint` as appropriate.

Recording contract (the gate's input format):
- Record a `bypass_attempts[]` entry for every condition you actually attempted, citing the actual harness path + test name in `attempt_summary`. `outcome` follows the run: `no_finding` if the assertion held, `partial_evidence` if you observed unexpected state but didn't reach a fund-loss condition, `finding_recorded` (with `finding_id`) when you recorded a finding via `bob_record_candidate_claim`, or `blocked` when the harness couldn't run.

Cross-surface pivots and composition conjectures:
- When the assigned package exposes a credible cross-stack pivot that crosses a trust boundary (a bridge mint/burn edge, a Wormhole/LayerZero message verifier, an IBC relayer hop, a cross-chain object/coin transfer, a web→on-chain identity or value path), call `bob_propose_transition` with `from_surface`, `to_surface`, a closed-enum `transition_kind`, and a bounded `trust_assumption`. The orchestrator routes the Transition node into the affected Surface briefs.
- When you form a composition conjecture worth dispatching — two surfaces whose individual findings or behaviors compose into impact neither reaches alone — call `bob_propose_hypothesis` with a `hypothesis_statement` describing the A→B composition and `surface_refs` listing the surfaces touched, then call `bob_attach_contract` to bind the witness predicates + production paths + invariants to the minted node and clear the dispatcher's satisfiability gate.
...
Coverage:
- Call `bob_log_coverage` after meaningful tests with `endpoint` set to `<address>::<module>::<function>` (Aptos) or `<package_id>::<module>::<function>` (Sui), `bug_class` from the Move bug class catalog above, and `status` from `tested|blocked|promising|needs_auth|requeue`.
...
- `bob_read_static_analysis_index({ target_domain })` surfaces pre-computed static hits (Move Prover / linter findings the inventory walker already ran) keyed to source files — read it first so flagged capability-leak / unchecked-arith / friend-boundary lines seed a concrete bypass condition instead of a cold module read. When a local analyzer produced a SARIF report (a `--build` analyzer run, an imported CI artifact), ingest it via `bob_ingest_sarif({ target_domain, ... })` so those results join the index and become bypass-loop leads.
...
- Read `surface.ink_harness_path` (or `surface.cargo_harness_path`, then `surface.move_harness_path`, whichever the platform spec populates) for the ink! contract source root — a directory containing `Cargo.toml` at the root with `[lib] crate-type = ["cdylib"]` and an `#[ink::contract]` module, OR a workspace root with multiple crates. If none of those keys is set, no `cargo test` PoC can be scaffolded — record `blocked_harness_runs[{ kind: "substrate_fork", harness: "missing-ink-harness", reason: "no ink!/cargo harness path set (surface.ink_harness_path / cargo_harness_path)" }]` and set `surface_status: partial`.
...
For this surface the bar is a `bob_substrate_run` whose test passes under the evaluator convention (`tests[].status: Pass` = bug reproduced) and holds on the control.

Consider — any one of these is a way in; pick by the selectors + storage layout you read, do not walk them in order. Use the conditions in `bob_spec_status.trust_assumptions[*].bypass_conditions` when present; otherwise derive them from selectors + storage layout. For each you pursue, articulate the concrete cross-contract or selector-call sequence the bypass exercises before you scaffold a test for it. Substrate / ink! bug class catalog:
...
Prerequisites (causal — these are the orderings the work imposes, not a checklist; do each before what depends on it):
- Enumerate the assigned contract's selectors and storage layout. Read `pallet_contracts.ContractInfoOf` for the address via `bob_substrate_fetch_storage` to get the `code_hash` (the BLAKE2-256 hash of the WASM blob). Pair this with the harness sources to map selectors → functions. The `#[ink(message)]`, `#[ink(message, payable)]`, and `#[ink(constructor)]` attributes mark the public attack surface; selectors are derived from the function name's BLAKE2-256 hash truncated to 4 bytes — you cannot scaffold a test against selectors you have not enumerated.
- Build the live trust map. For every privileged function you find, identify which storage cell gates it (typically an `owner: AccountId`, `admin: Mapping<AccountId, ()>`, or role-bitmap cell). Fetch the current value via raw storage. Confirm the migration / upgrade authority: `pallet_contracts` does not natively support upgrades, so a contract that exposes `set_code_hash(new: Hash)` is its own upgrade authority — verify it is admin-gated and the admin is not an attacker-controlled signer.
- Scaffold an ink! test under `harness_path/lib.rs` (use `Write` for the `.rs` file, or extend an existing `#[cfg(test)] mod tests`). Use `#[ink::test]` for offline tests (in-VM) or `#[ink_e2e::test]` for E2E tests against a node. Pure-VM `#[ink::test]` tests run inside a deterministic mock environment with no real network access — `cargo test` does NOT clone mainnet state, but the harness can read `BOB_SUBSTRATE_FORK_URL` from env if it opts into chopsticks-fork or similar. The `match_test` you record in `sc_evidence` MUST exactly match the test function name; the runner uses `cargo test ... --exact <match_test>` so partial matches will not run.
- Run the test via `bob_substrate_run`. Inspect `tests[].status` (`Pass` = bug reproduced under the evaluator convention), `tests[].test_id`, `tests[].reason`. If `ok: false` with `reason: substrate_not_in_path` / `substrate_dependency_missing` / `cargo_compile_failed`, `reason: "rpc_unreachable"`, a reason starting with `no_fork_endpoints`, populated `rpc_policy_rejections[]`, or all `fork_attempts[]` failed with RPC errors, set `surface_status: partial` and record `blocked_harness_runs[]` with `kind: substrate_fork` or `rpc_endpoint` as appropriate.

Recording contract (the gate's input format):
- Record a `bypass_attempts[]` entry for every condition you actually attempted, citing the actual harness path + test name in `attempt_summary`. `outcome` follows the run: `no_finding` if the assertion held, `partial_evidence` if you observed unexpected state but didn't reach a fund-loss condition, `finding_recorded` (with `finding_id`) when you recorded a finding via `bob_record_candidate_claim`, or `blocked` when the harness couldn't run.

Cross-surface pivots and composition conjectures:
- When the assigned contract exposes a credible cross-stack pivot that crosses a trust boundary (an XCM cross-chain message edge, a bridge mint/burn, a chain-extension into runtime-managed assets, a cross-contract or web→on-chain identity or value path), call `bob_propose_transition` with `from_surface`, `to_surface`, a closed-enum `transition_kind`, and a bounded `trust_assumption`. The orchestrator routes the Transition node into the affected Surface briefs.
- When you form a composition conjecture worth dispatching — two surfaces whose individual findings or behaviors compose into impact neither reaches alone — call `bob_propose_hypothesis` with a `hypothesis_statement` describing the A→B composition and `surface_refs` listing the surfaces touched, then call `bob_attach_contract` to bind the witness predicates + production paths + invariants to the minted node and clear the dispatcher's satisfiability gate.
...
Coverage:
- Call `bob_log_coverage` after meaningful tests with `endpoint` set to `<contract_address>::<selector_name>` (e.g., `5GrwvaEF...::transfer`), `bug_class` from the substrate / ink! bug class catalog above, and `status` from `tested|blocked|promising|needs_auth|requeue`.
...
- `bob_read_static_analysis_index({ target_domain })` surfaces pre-computed static hits (cargo-geiger / clippy / dylint findings the inventory walker already ran) keyed to source files — read it first so flagged unchecked-arith / set_code_hash / caller-spoof lines seed a concrete bypass condition instead of a cold source read. When a local analyzer produced a SARIF report (a `--build` analyzer run, an imported CI artifact), ingest it via `bob_ingest_sarif({ target_domain, ... })` so those results join the index and become bypass-loop leads.
...
For this surface the bar is a `bob_cosmwasm_run` cw-multi-test whose test passes under the evaluator convention (`tests[].status: Pass` = bug reproduced) and holds on the control. A test that passes in cw-multi-test but would not reproduce on real wasmd (gas-metering or chain-state differences) is `partial_evidence`, not a finding.

Consider — any one of these is a way in; pick by the contract schema + storage layout you read, do not walk them in order. Use the conditions in `bob_spec_status.trust_assumptions[*].bypass_conditions` when present; otherwise derive them from the schema + storage layout. For each you pursue, articulate the concrete ExecuteMsg / sub-message / migrate sequence the bypass exercises before you scaffold a test for it. CosmWasm bug class catalog:
...
Coverage:
- Call `bob_log_coverage` after meaningful tests with `endpoint` set to `<contract_address>::<msg_variant>` (e.g., `osmo1...::Execute::Withdraw`), `bug_class` from the CosmWasm bug class catalog above, and `status` from `tested|blocked|promising|needs_auth|requeue`.
...
- `bob_read_static_analysis_index({ target_domain })` surfaces pre-computed static hits (cargo-geiger / clippy / cosmwasm-check findings the inventory walker already ran) keyed to source files — read it first so flagged migrate-open / non-payable / reply-misuse lines seed a concrete bypass condition instead of a cold source read. When a local analyzer produced a SARIF report (a `--build` analyzer run, an imported CI artifact), ingest it via `bob_ingest_sarif({ target_domain, ... })` so those results join the index and become bypass-loop leads.
...
After the final-round write succeeds and the readback confirms it, call `bob_write_proof_bundle` once when any final-reportable finding has a usable replay, invariant, or differential proof handle from your final replay work. Include only `reportable: true` final findings, and bind each pack to that finding's own handle: `replay_script` uses only `bob_repo_docker_run` handles created for that finding with `replay_context.finding_id` equal to the final `F-N` id plus the replay command; if an otherwise usable replay handle lacks that binding, rerun the same replay command with final-round `replay_context` before writing the bundle. `invariant` uses only reproducing invariant `run_hash` rows whose `finding_id` is the same Bob `F-N` id — read `bob_read_invariant_runs({ target_domain })` to pick those reproducing `run_hash` rows (including any minted by your own `bob_halmos_run` re-execution) — and `differential` uses the C10 differential row for the same finding. If there are no eligible proof handles, do not invent a bundle; say the blocker in the final summary. If `bob_write_proof_bundle` fails, read the structured error and either fix the pack input or report why no proof bundle was written. Never write `proof-bundles.json` or `proof-bundles.md` via Bash.
...
SHAPE CONSTRAINTS on `bob_compose_report` (anchors are `mcp/tools/compose-report.js` unless stated; re-check the cited symbol, not the number, if the file drifts):
...
- A `# …` line inside prose is USUALLY inert but is NOT always — the state that decides it is whether a finding is already open when the parser reaches that line. The H1 branch (`mcp/core/audit-report-parser.js:151`) is gated on `currentFinding == null`. In the common case a finding IS already open — `## ${heading}` at `:1096` opened one, or `## Severity Summary` at `:1090` did — so the branch is skipped and the line is buffered as body text at `:189`: document title unchanged, finding count unchanged, just broken-looking output. But when the FIRST section's heading matches the exemption regex at `:166` AND no severity summary is rendered, prose is reached with `currentFinding` still `null`: the branch at `:151` fires, `title` is already set from bob's own report H1, control falls to the `else` at `:157-159`, and `startFinding(candidate)` MINTS A FINDING titled with your line. Verified by execution: sections `[Scope — Estate and Systems Assessed (whose prose contains a `# Injected H1` line), F-1 Real]` with `severity_summary` empty parses to 3 findings — `Injected H1`, `F-1 Real`, `CVSS / CWE (informational)` — against 2 for the identical document without that one line. Never start a prose line with `#`.
- `repro_steps_by_finding[].finding_id` is a plain 64-char string (`:821`). It is never matched against `FINDING_ID_RE` (zero references in `compose-report.js`, positive control `SECTION_KINDS` = 5 hits in that same file; `FINDING_ID_RE` itself resolves at only 4 sites tree-wide — `mcp/core/io/identifier-contracts.js:3`, `:9`, `mcp/core/io/validation.js:5`, `:249` — the 48-hit figure often quoted is the `FINDING_ID_RE|parseFindingId` alternation, 44 of which are `parseFindingId`) and it is never checked against the finding ledger. Bob will write `### F-9` for an `F-9` that was never recorded. Pass only ids you have actually recorded.
- Never read an empty `parser_warnings` as a clean bill of health. `mcp/core/audit-report-parser.js` has exactly one `warnings.push` site (`:200`), guarded by `findings.length === 0` (`:199`); positive control, that file has 7 `.push(` sites in total. For any document yielding at least one finding, `[]` is structurally FORCED, not observed — it means "no check could fire here", not "checked and clean".
...
3. For each REPORTABLE finding (filtered by the gate above), branch first by `finding.capability_pack`, then by `finding.surface_type`:
   ...
   **HTTP findings** (`surface_type: "web"` or null):
   ...
   - CWE: render the finding's persisted `cwe` value verbatim. It is required and catalog-validated for medium+ findings (validated against `mcp/core/scoring/cwe-catalog.js` at write time — common web ids: CWE-79 XSS, CWE-639 IDOR, CWE-352 CSRF, CWE-918 SSRF, CWE-200 info exposure), so do NOT re-classify, pick, or substitute a different id at report time — the recording already froze it. For a legacy row whose `cwe` is absent or invalid, render an explicit "CWE unavailable (legacy record)" marker rather than inventing one.
   - Endpoint
   ...
   **Smart-contract findings** (`surface_type: "smart_contract"`):
   ...
   - CWE: render the finding's persisted `cwe` value verbatim. It is required and catalog-validated for medium+ findings (validated against `mcp/core/scoring/cwe-catalog.js` at write time), so do NOT re-classify, pick, or substitute a different id at report time — the recording already froze it. For a legacy row whose `cwe` is absent or invalid, render an explicit "CWE unavailable (legacy record)" marker rather than inventing one. The canonical mappings below mirror `mcp/core/scoring/cwe-catalog.js` SMART_CONTRACT_FAMILY_CWE for REFERENCE only — to recognize the expected id, not to re-derive it; families share these unless noted:
