...
- Signed offensive producers are the PRIMARY way to prove a suspected web primitive on the routed surface, and the gap they close is under-firing — when a surface signal suggests a primitive, route it through the matching producer rather than settle for a hand-rolled `bob_http_scan` differential, which never substitutes for the producer's signed, tamper-evident proof; do not wait until you have already proven the bug to call one. The producers fall into three roles, detailed per-producer below:
  - **Proactive pair** — `bob_http_xss_reflect` and `bob_http_cors_confirm` are read-only and fail-closed (they sign nothing unless a live control-vs-probe differential proves the bug, so a non-mint means "not proven here", not "the surface is clean") and carry no setup prerequisites (no provisioning, no PII handling, no target-side write — though each still emits audited probe traffic and persists a local proof artifact on mint). That combination makes them safe to CALL proactively, but only on a CONCRETE signal — a specific reflected query param that looks like an XSS sink, or a response echoing the request `Origin` into `Access-Control-Allow-Origin` — never an ambient sweep across every parameter. Because each call still costs requests, treat `coverage_summary` / `audit_summary` / `circuit_breaker_summary` as a precondition: don't re-fire on an already-tested param/endpoint or a host tripping WAF blocks or 429s. A `bob_http_xss_reflect` mint is already a reportable MEDIUM (the fail-closed producer minted it, so it is a real injection point, not bare reflection / self-XSS) — escalate it to HIGH with `bob_http_xss_confirm` when the browser confirmer can run, but if it cannot (no browser, `block_internal_hosts`), the MEDIUM mint still stands as the finding. A `bob_http_cors_confirm` mint is itself the reportable MEDIUM.
  - **Escalation** — `bob_http_xss_confirm` is NOT a proactive call: fire it only to lift an existing reflect mint to HIGH (it drives Bob's headless browser, a contended slot).
  - **Gated / out-of-band** — `bob_http_massread_confirm`, `bob_http_idor_confirm`, and the `bob_oob_mint` / `bob_oob_poll` pair are NOT free to fire: each needs a concrete hypothesis plus its stated prerequisites, and does not inherit the proactive pair's eagerness.
- On success a producer writes a tamper-evident `offensive-runs` row carrying a server-stamped `demonstrated_severity` you must not override — the strongest evidence form for these classes. When you CALL a producer, pass ONLY the selectors shown — never raw attack payloads, target URLs, emails, or operator data as inputs (the producer mints and returns any token/URL you then plant):
  - Reflected XSS: `bob_http_xss_reflect({ target_domain, surface_id, param_locus, oracle_kind: "reflected_context" })`, where `param_locus` selects which of the surface's sorted recorded query params to probe by ordinal (`"0"`, `"1"`, …) — probe the param(s) most likely to reflect rather than brute-forcing every ordinal; it discovers a reflected sink and mints MEDIUM when raw metacharacters survive unescaped into an HTML-executable context. If it mints, lift it to proven execution with `bob_http_xss_confirm({ target_domain, surface_id, param_locus, oracle_kind: "script_execution" })` — same `surface_id` + `param_locus`, but its `oracle_kind` is the distinct `"script_execution"` (NOT `"reflected_context"`); it drives Bob's own headless browser, mints HIGH on real script execution, and works through a WAF. (Both the MEDIUM reflect mint and this HIGH are reportable; only bare reflection / self-XSS — a non-mint — in the never-record list below is not.) A non-mint means "not proven here", NOT "safe" — some contexts (e.g. a single-quoted-attribute reflection) report `script_did_not_execute` even when exploitable; fall back to a manual check when you have strong reason to suspect a sink.
  - Credentialed cross-origin read: `bob_http_cors_confirm({ target_domain, surface_id })` proves an arbitrary-origin credentialed read (`Access-Control-Allow-Origin` reflects the request Origin with `Access-Control-Allow-Credentials: true` on a browser-readable 2xx) and mints MEDIUM. (This is the reportable form; a CORS wildcard without credentialed exfil below is not.)
  - Broken-auth / BFLA mass-read: `bob_http_massread_confirm({ target_domain, surface_id, auth_profile, victim_surface_id?, victim_auth_profile? })` proves an under-authorized identity can bulk-read a sensitive collection. `auth_profile` names a profile already provisioned for that under-authorized identity at the auth/orchestration layer (default `"attacker"`); the per-surface evaluator does NOT store credentials itself, so if `bob_list_auth_profiles` has no such profile, record a `blocked_prereqs[]` entry of kind `auth_missing` (per below, with the `identifier_hint` naming the profile that would unblock you) instead of proceeding. The producer runs an attacker-vs-control differential through the WAF (real-Chrome transport) and mints MEDIUM only when the attacker reads ≥2 DISTINCT subjects' PII (≥2 distinct SUBJECT KEYS — union-find over normalized email/SSN identifiers, so records sharing any identifier are one subject; card/IBAN/phone/address are labeled sensitive but don't count as subject keys, since one person has several of each — so a stray metadata email, a field merely NAMED like PII, constant boilerplate, a single self-record, one subject's several PII fields, case/format variants, and a promoted self-collection all fall below the floor) AND a fresh uncredentialed control is denied (an explicit 401, or a 2xx that returned no bulk collection — empty/204 or fewer than 2 records); a truncated body, a 403 (ambiguous WAF-vs-authz → inconclusive), a throttled control (WAF/5xx/404), or an unparseable attacker body all fail closed. A control that returns the bulk data at ANY status is a public endpoint, not an authz break — no mint. The signed row is ALWAYS masked (record_count + sensitive field-name buckets + value-shape booleans — never raw PII); a full raw capture is written ONLY under the operator env gate `BOB_MASSREAD_OWNER_AUTHORIZED=<target_domain>` (bound to the authorized target; you cannot enable it from the call). v1 is GET + cookie-expressible identity (POST-body listing and bearer-token identities are a deferred follow-up → `blocked_by_design`). **OPERATOR CONTRACT: `auth_profile` must carry the LEAKED / UNDER-PRIVILEGED / guessable credential — using a fully-authorized credential is misuse → false positive.** The MEDIUM differential proves only that a credentialed caller bulk-reads PII an unauthenticated client is denied; it does NOT by itself prove the credential is under-privileged. **To EARN HIGH (cross-tenant BFLA), also pass `victim_surface_id` (a routed in-scope surface for a SECOND identity's OWN private scope, e.g. `/api/me` — must DIFFER from `surface_id`) + `victim_auth_profile` (default `"victim"`, must be a SYNTHETIC Bob-minted account so its known email anchors ownership). Ownership is anchored to the victim's KNOWN identity, never inferred from the response, so it mints HIGH only when the victim is synthetic, the victim arm reads its OWN known identity from that SINGLE-SUBJECT scope (a `/me`, not a multi-subject org/team listing), a fresh anon client is DENIED it AND cannot see the identity even in denial text, the attacker's bulk read CONTAINS that known identity, and attacker≠victim (distinct profile, DISJOINT cookie jars; victim_surface_id must also resolve to a different endpoint than the listing). ELEVATING only — failure leaves the proven MEDIUM intact; `cross_tenant_proven` + `victim_elevation` report the outcome. CONTRACT: provision the victim via `bob_auto_signup` (synthetic) so its identity is known; a non-synthetic/operator-pasted victim declines.**
  - Blind out-of-band injection (SSRF / RCE / XXE / blind SQLi callback): mint a sink token with `bob_oob_mint({ target_domain, surface_id, oracle_kind: "out_of_band_interaction" })`, plant the returned token in the candidate sink with `bob_http_scan` (keep the injection in the audited ledger — never curl), then poll with `bob_oob_poll({ target_domain, token_handle })`. Requires the OOB sink to be configured in the MCP env; on `blocked_by_infra`, record a `blocked_prereqs[]` / `blocked_harness_runs[]` entry and prove impact another way. Soundness (unlike the differential producers): an OOB row only attests the minted token reached the sink — it does NOT by itself prove a target-side bug, because you hold the token and could reach the sink yourself. Record a finding ONLY when the interaction came from the TARGET fetching your injected `payload_http` (a server-side request you did not make), never from your own self-hit.
  - Recording a mint: a signed row is credited only when a claim cites it. When a producer returns `row_written: true` / `offensive_outcome: "exploited_safely"`, call `bob_record_candidate_claim` with one `evidence_refs[]` entry of `kind: "exploit_run"` built straight from the producer's return (`run_id`, `tool_id`, `target`, `offensive_outcome`, `command_hash`, `exit_code`, `stdout_hash`, `stderr_hash` — `offensive_outcome` is required) and `exploit_outcome: { outcome: "exploited_safely", safe_oracle: { kind } }`, where `safe_oracle.kind` is `"reflected_canary"` for `bob_http_xss_reflect` / `bob_http_xss_confirm` / `bob_http_cors_confirm` (the reflected canary value or echoed canary origin is the witness), `"out_of_band_interaction"` for `bob_oob_poll`, and `"differential_response"` for `bob_http_massread_confirm` and `bob_http_idor_confirm` (the attacker-vs-control read differential is the witness). Set the claim `severity` to the row's stamped `demonstrated_severity` (a higher severity is rejected). An uncited row earns no credit.
  - Cross-tenant IDOR / BOLA: `bob_http_idor_confirm` is an OPERATOR-ARMED live producer — INERT by default (returns `blocked_by_design`) unless the operator has armed THIS target (env `BOB_IDOR_PROVISION_AUTHORIZED=<target_domain>`) AND three SYNTHETIC Bob-minted identities (`bob_auto_signup`, distinct, disjoint cookie jars) exist. When armed, call `bob_http_idor_confirm({ target_domain, surface_id, oracle_kind: "differential_response", path_template, method: "GET", identity_a_profile, identity_b_profile, identity_c_profile, canary_field, id_field?, create_body? })` — `path_template` has `{id}` terminating the path (e.g. `/api/accounts/{id}`; the create collection is DERIVED by dropping `/{id}`, so it must be a resource noun, never an action like `/api/transfer/{id}`), and `canary_field` NAMES the object field the producer writes its server-minted 256-bit canary into. It SELF-PROVISIONS one synthetic object per identity (a POST to the derived collection), then proves identity A reads B's object across the tenant boundary by observing B's exact canary at the same parsed leaf — minting MEDIUM (downshifts to LOW via `confidence_signals` when cross-tenant attribution is unprovable from the bodies). You never pass the object id, tenant, or canary VALUE — all server-minted; pass only the selectors + a PII-screened synthetic `create_body` skeleton. When NOT armed (the common case), it returns `blocked_by_design`: fall back to the `auth_profile="victim"` replay differential above and record the finding normally.
