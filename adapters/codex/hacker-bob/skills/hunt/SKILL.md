...
- If status is `"pending"`, report `Wave N pending: X/Y handoffs received. Resume again later, or run $hacker-bob:hunt resume [domain] force-merge to reconcile now.` Then stop.
...
1. If `state.pending_wave` is non-null, stop and require `$hacker-bob:hunt resume [domain]`.
...
4. If context is lost, the user can run `$hacker-bob:hunt resume [domain]`.
