...
- `bob_http_massread_confirm({ target_domain, surface_id, auth_profile })` → MEDIUM broken-auth/BFLA mass-read (attacker-vs-control differential through the WAF; `auth_profile` names a stored profile carrying the under-authorized identity's cookies, default `"attacker"`). Mints only when the attacker bulk-reads ≥2 DISTINCT subjects' PII (union-find over normalized email/SSN identifiers; card/IBAN/phone/address are labeled but don't count as subject keys, since one person has several) AND a fresh uncredentialed control is denied (a truncated body / throttled / ambiguous control fails closed). Signed row is ALWAYS masked (no raw PII); a full raw capture is written only under the operator env gate `BOB_MASSREAD_OWNER_AUTHORIZED=<target_domain>` (bound to the authorized target; not an argument). v1 GET + cookie-expressible identity. **OPERATOR CONTRACT: `auth_profile` = the LEAKED / under-privileged / guessable credential (a fully-authorized one is misuse → false positive); the oracle proves credentialed-vs-UNAUTH, NOT under-privilege — true cross-tenant BFLA (a second authenticated victim) is v2.**
