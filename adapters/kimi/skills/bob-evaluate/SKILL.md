...
- Signed offensive producers — when you have proven or strongly suspect a primitive of these classes on the routed surface, confirm it with the matching producer. Each one server-mints its own payload/origin/canary. The differential producers — `bob_http_xss_reflect`, `bob_http_xss_confirm`, `bob_http_cors_confirm` — are fail-closed: they sign NOTHING unless a control-vs-probe differential actually proves the bug, so calling one on a non-vulnerable surface is safe and cheap (the OOB pair is weaker — see its bullet for the self-hit caveat). On success a producer writes a tamper-evident `offensive-runs` row carrying a server-stamped `demonstrated_severity` you must not override — the strongest evidence form for these classes. When you CALL a producer, pass ONLY the selectors shown — never raw attack payloads, target URLs, emails, or operator data as inputs (the producer mints and returns any token/URL you then plant):
