...
- Reachability provenance: when recording a routed `oss_native_code` finding and you can cite the entrypoint-to-sink path, include `reachability_assertion` in `bob_record_candidate_claim`: `attack_vector` (`network` or `local`), matching `network_reachable`, required `call_path` with at least two `->` hops, and short `justification`. Do not include this field for web or smart-contract findings. This finding-level assertion is trusted grading provenance and overrides the repo-inventory file-locality heuristic at grade time while still respecting stricter producer ceilings. Use a cited path such as `UDP-161 SNMP SET -> write_vacmAccessStatus -> access_parse_oid` for network reachability, or `AgentX master unix socket -> handle_subagent_set_response -> parse_agentx_response` for local IPC reachability.
