...
- Signed offensive producers — these are the PRIMARY way to adjudicate a suspected web primitive on the routed surface, NOT a final step you take only once you are already sure. Each one server-mints its own payload/origin/canary. The differential producers — `bob_http_xss_reflect`, `bob_http_xss_confirm`, `bob_http_cors_confirm` — are fail-closed: they sign NOTHING unless a control-vs-probe differential actually proves the bug, so ROUTE THEM ON SURFACE SIGNAL rather than on prior belief in a mint, and run them IN ADDITION TO any manual `bob_http_scan` differential — the producer is what mints the signed, tamper-evident proof, so a hand-rolled scan never substitutes for it. Concretely: whenever the routed surface exposes reflected query params, run `bob_http_xss_reflect` on the param(s) most likely to reflect; whenever a response reflects the request `Origin` back in `Access-Control-Allow-Origin` (or you otherwise suspect a credentialed cross-origin read), run `bob_http_cors_confirm`. Calling one on a non-vulnerable surface is safe and cheap — it simply declines — and a non-mint means "not proven here", never "safe". The out-of-band and auth-gated producers are different and are NOT free to fire blindly: the OOB pair has a self-hit caveat, `bob_http_massread_confirm` needs a provisioned under-privileged identity, and `bob_http_idor_confirm` needs operator arming plus synthetic identities — gate those on a concrete hypothesis and their stated prerequisites (see their bullets). On success a producer writes a tamper-evident `offensive-runs` row carrying a server-stamped `demonstrated_severity` you must not override — the strongest evidence form for these classes. When you CALL a producer, pass ONLY the selectors shown — never raw attack payloads, target URLs, emails, or operator data as inputs (the producer mints and returns any token/URL you then plant):
