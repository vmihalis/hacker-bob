...
- Signed offensive producers — when you have proven or strongly suspect a primitive of these classes on the routed surface, confirm it with the matching producer. Each one server-mints its own payload/origin/canary. The differential producers — `bob_http_xss_reflect`, `bob_http_xss_confirm`, `bob_http_cors_confirm` — are fail-closed: they sign NOTHING unless a control-vs-probe differential actually proves the bug, so calling one on a non-vulnerable surface is safe and cheap (the OOB pair is weaker — see its bullet for the self-hit caveat). On success a producer writes a tamper-evident `offensive-runs` row carrying a server-stamped `demonstrated_severity` you must not override — the strongest evidence form for these classes. When you CALL a producer, pass ONLY the selectors shown — never raw attack payloads, target URLs, emails, or operator data as inputs (the producer mints and returns any token/URL you then plant):
  ...
  - Broken-auth / BFLA mass-read: `bob_http_massread_confirm({ target_domain, surface_id, auth_profile })` proves an under-authorized identity can bulk-read a sensitive collection. First store that identity's cookies in an auth profile (via `bob_auth_store`); `auth_profile` names it (default `"attacker"`). The producer runs an attacker-vs-control differential through the WAF (real-Chrome transport) and mints HIGH only when the attacker reads ≥2 records carrying a sensitive field AND a fresh uncredentialed control is denied (401/403, or a 2xx with 0 records). A control that ALSO reads the bulk data is a public endpoint, not an authz break — no mint. The signed row is ALWAYS masked (record_count + sensitive field-name buckets + value-shape booleans — never raw PII); a full raw capture is written ONLY under the operator env gate `BOB_MASSREAD_OWNER_AUTHORIZED=1` (you cannot enable it from the call). v1 is GET + cookie-expressible identity (POST-body listing and bearer-token identities are a deferred follow-up → `blocked_by_design`).
...
- `bob_http_massread_confirm({ target_domain, surface_id, auth_profile })` → HIGH broken-auth/BFLA mass-read (attacker-vs-control differential through the WAF; `auth_profile` names a stored profile carrying the under-authorized identity's cookies, default `"attacker"`). Mints only when the attacker bulk-reads ≥2 sensitive records AND a fresh uncredentialed control is denied. Signed row is ALWAYS masked (no raw PII); a full raw capture is written only under the operator env gate `BOB_MASSREAD_OWNER_AUTHORIZED=1` (not an argument). v1 GET + cookie-expressible identity.
