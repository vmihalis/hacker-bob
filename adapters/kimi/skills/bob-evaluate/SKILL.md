...
- Signed offensive producers are the PRIMARY way to prove a suspected web primitive on the routed surface — when you suspect one, route that signal through the matching producer instead of settling for a hand-rolled `bob_http_scan` differential (which never substitutes for the producer's signed, tamper-evident proof). The gap they close is under-firing: do not wait until you have already proven the bug to call one. The read-only differential producers — `bob_http_xss_reflect` and `bob_http_cors_confirm` for discovery, plus their escalation `bob_http_xss_confirm` — are all fail-closed: they sign nothing unless a live control-vs-probe differential actually proves the bug (a non-mint means "not proven here", never "safe"). Only the two discovery producers are also free to fire — no provisioning, no PII handling, no write side-effect, no self-hit — and it is fail-closed AND free-to-fire together that makes routing them proactively on surface signal safe. `bob_http_xss_confirm` is an escalation, not a proactive call: use it to lift a reflect mint to HIGH, and because it drives Bob's headless browser (a contended slot) do not spray it across sinks. A producer that is gated or not fail-closed does not inherit this eagerness. Route on a concrete suspected sink, not reflexively across every parameter: when a specific reflected query param looks like a plausible XSS sink, run `bob_http_xss_reflect` on that param (a reflect-only mint is never the reportable form); when a response reflects the request `Origin` in `Access-Control-Allow-Origin`, run `bob_http_cors_confirm`. Keep honoring `coverage_summary`, `audit_summary`, and `circuit_breaker_summary` — don't re-fire a producer on a param/endpoint already tested or against a host already tripping WAF blocks or 429s. The out-of-band and auth-gated producers are NOT free to fire and each needs a concrete hypothesis plus its stated prerequisites: the OOB pair (self-hit caveat), `bob_http_massread_confirm` (a provisioned under-privileged identity), and `bob_http_idor_confirm` (operator arming + synthetic identities). On success a producer writes a tamper-evident `offensive-runs` row carrying a server-stamped `demonstrated_severity` you must not override — the strongest evidence form for these classes. When you CALL a producer, pass ONLY the selectors shown — never raw attack payloads, target URLs, emails, or operator data as inputs (the producer mints and returns any token/URL you then plant):
  - Reflected XSS: `bob_http_xss_reflect({ target_domain, surface_id, param_locus, oracle_kind: "reflected_context" })`, where `param_locus` selects which of the surface's sorted recorded query params to probe by ordinal (`"0"`, `"1"`, …) — probe the param(s) most likely to reflect rather than brute-forcing every ordinal; it discovers a reflected sink and mints MEDIUM when raw metacharacters survive unescaped into an HTML-executable context. If it mints, lift it to proven execution with `bob_http_xss_confirm({ target_domain, surface_id, param_locus, oracle_kind: "script_execution" })` — same `surface_id` + `param_locus`, but its `oracle_kind` is the distinct `"script_execution"` (NOT `"reflected_context"`); it drives Bob's own headless browser, mints HIGH on real script execution, and works through a WAF. (This is the reportable form; bare reflection / self-XSS in the never-record list below is not.) A non-mint means "not proven here", NOT "safe" — some contexts (e.g. a single-quoted-attribute reflection) report `script_did_not_execute` even when exploitable; fall back to a manual check when you have strong reason to suspect a sink.
...
If your dispatched node's allow-list includes the signed offensive producers, they mint tamper-evident `offensive-runs` rows (server-minted payloads — pass only selectors, never raw payloads or operator data). The read-only differential producers (`xss_reflect` / `xss_confirm` / `cors_confirm`) are fail-closed — they sign nothing unless a control-vs-probe differential proves the bug. When your dispatched Contract's `production_paths[].tool_call_pattern[]` calls for one of these (and `allowed_tools_for_node[]` permits it), prefer the signed producer over a hand-rolled `bob_http_scan` differential — the producer is what mints the signed proof. The Contract's witness path is the execution authority here: do not fire producers your Contract did not request just because a surface signal is present (that surface-signal routing is the wave-scheduler evaluator's mode, not this shell's). The OOB pair is weaker (self-hit caveat below) and the auth-gated producers need their stated prerequisites, so gate those on a concrete hypothesis. The exact call shape and the distinct `oracle_kind` per tool matter (a wrong `oracle_kind` is a schema rejection):
