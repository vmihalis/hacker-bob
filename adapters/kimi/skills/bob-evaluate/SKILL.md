...
- `bob_http_massread_confirm({ target_domain, surface_id, auth_profile })` → HIGH broken-auth/BFLA mass-read (attacker-vs-control differential through the WAF; `auth_profile` names a stored profile carrying the under-authorized identity's cookies, default `"attacker"`). Mints only when the attacker bulk-reads ≥2 records carrying a real PII VALUE AND a fresh uncredentialed control is denied (a truncated body / throttled / ambiguous control fails closed). Signed row is ALWAYS masked (no raw PII); a full raw capture is written only under the operator env gate `BOB_MASSREAD_OWNER_AUTHORIZED=1` (not an argument). v1 GET + cookie-expressible identity. **OPERATOR CONTRACT: `auth_profile` = the LEAKED / under-privileged / guessable credential (a fully-authorized one is misuse → false positive); the oracle proves credentialed-vs-UNAUTH, NOT under-privilege — true cross-tenant BFLA (a second authenticated victim) is v2.**
