...
HEADLESS_MCP_READINESS_PROMPT = (
    ...
    "If the system says MCP tools are still connecting or deferred, do not answer "
    "with a status update and do not ask the operator for guidance. Wait briefly "
    "with Bash sleep, then retry the Bob MCP action until the tools are available "
    "or the process deadline stops you. Never inspect protected Bob session files "
    ...
    "operator. Derive target_domain as the URL hostname exactly (ASCII-normalized, "
