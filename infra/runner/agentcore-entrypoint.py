...
HEADLESS_MCP_READINESS_PROMPT = (
    ...
    "If the Bob tool you need is not already visible, immediately call ToolSearch "
    "with select:mcp__hacker-bob__<exact_tool_name>, then invoke the returned tool; "
    "do not wait for the tool to appear in the initial list. If the system says the "
    "MCP server is still connecting, wait briefly with Bash sleep and repeat that "
    "ToolSearch. Do not answer with a plan or status update and do not ask the "
    ...
    "operator. When --private-targets is present, the operator-side BOB_LAB_TARGET "
    "and BOB_LAB_TARGET_ACK attestation is already provisioned in the MCP child; "
    "never infer that it is missing and never ask for it to be configured. Attempt "
    "bob_init_session, and treat only its actual structured MCP error as authority truth. "
