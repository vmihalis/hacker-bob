...
Every MCP tool response is wrapped in a standard envelope: success responses use `{ "ok": true, "data": ..., "meta": { "tool": "...", "version": 1 } }`; failures use `{ "ok": false, "error": { "code": "...", "message": "..." }, "meta": ... }`. Prompts and integrations should read successful payloads from `.data`. Direct JS callers of `executeTool(...)` receive this plain envelope object and should inspect `.ok`, `.data`, and `.error`; do not call `JSON.parse(await executeTool(...))`. The MCP transport serializes the envelope into response text for MCP clients.

Bob also records operational MCP tool-call telemetry to `~/bounty-agent-telemetry/tool-events.jsonl` by default. The ledger stores tool names, timing, outcome codes, safe correlation fields, and registry metadata only; it does not store raw tool arguments, headers, bodies, credentials, auth values, response bodies, or static artifact content. Set `BOUNTY_TELEMETRY=0` to disable writes or `BOUNTY_TELEMETRY_DIR=/path/to/dir` to move the ledger. Orchestrators can inspect read-time summaries with `bounty_read_tool_telemetry`.
...
Recon may enrich `attack_surface.json` surfaces with optional `surface_type`, `bug_class_hints`, `high_value_flows`, `evidence`, and `ranking` fields. The original required fields remain compatible: `id`, `hosts`, `tech_stack`, `endpoints`, `interesting_params`, `nuclei_hits`, and `priority`. MCP ranking computes runtime priority and reasons for status/brief views using API-ness, auth/admin/billing/data flows, GraphQL/WebSocket, object IDs, nuclei hits, JS secrets, imported traffic, and disclosed-report hints. Imports and public-intel fetches do not rewrite `attack_surface.json`; runtime ranking improves hunter prioritization only and does not add scanners, web3 automation, or cross-target memory.
...
- `attack_surface.json` — recon output grouped by priority, optionally enriched with surface type, likely bug classes, high-value flows, short evidence strings, and recon-provided ranking reasons; MCP ranking is computed at read time for prioritization
...
| **Public intel** | Network access to public program/report pages, or a provided program handle | Brief returns empty intel hints and runtime ranking uses local signals only |
